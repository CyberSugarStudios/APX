...
// ============================================================
        ...
        const WEAPON_PROPERTIES = [
            { key: "concealed", name: "Concealed", cost: 100, desc: "Advantage on AGI (Stealth) checks to hide this weapon. Attacks with this weapon against a Surprised creature deal an additional die of damage.", reqLabel: "Light Weapon", reqCheck: (ctx) => ctx.weightClass === 'light' },
            { key: "reach", name: "Reach", cost: 50, max: 2, desc: "You can hit creatures up to 2 extra squares further (1 square per purchase). On a Medium weapon, Reach only applies when wielded two-handed.", reqLabel: "Melee Weapon (Heavy, or Medium when 2H)", reqCheck: (ctx) => ctx.category === 'melee' && (ctx.weightClass === 'heavy' || ctx.weightClass === 'medium') },
            { key: "thrown", name: "Thrown", cost: 50, desc: "You can throw this weapon to make a ranged attack. For 300 additional Currency, it returns to you after your attack (Returning).", reqLabel: "One-Handed Weapon", reqCheck: (ctx) => ctx.weightClass !== 'heavy' },
            // Add-on for Thrown (shown under it in the forge)
            { key: "returning", name: "Returning (Thrown)", cost: 300, addonOf: "thrown", desc: "This weapon returns to you after your attack.", reqLabel: "Thrown", reqCheck: (ctx) => !!(ctx.props && ctx.props.thrown) && ctx.weightClass !== 'heavy' },
            ...
            { key: "crushing", name: "Crushing", cost: 300, desc: "On a hit, the creature must make a STR saving throw (DC 10 + your STR modifier) or be knocked Prone. If the target is already Prone, you deal an additional die of damage on a hit instead.", reqLabel: "Bludgeoning damage", reqCheck: (ctx) => ctx.effectiveDmgType === 'Bludgeoning' },
            { key: "flurry", name: "Flurry", cost: 300, desc: "When you hit a creature with this weapon, each subsequent attack you make against them costs 1 AP less (minimum 1 AP). You lose this AP reduction on a miss, or at the end of your turn.", reqLabel: "Slashing damage", reqCheck: (ctx) => ctx.effectiveDmgType === 'Slashing' },
            { key: "stunning", name: "Stunning", cost: 400, desc: "On a hit, the target must succeed on a CON saving throw (DC 10 + your STR modifier) or become Stunned until the end of your next turn. If you crafted this weapon to deal Energy (Electric) damage, you can use your INT modifier for the attack roll, damage roll, and DC.", reqLabel: "Two-Handed Weapon or Energy (Electric)", reqCheck: (ctx) => ctx.weightClass === 'heavy' || ctx.elemental === 'Electric' }
        ];
